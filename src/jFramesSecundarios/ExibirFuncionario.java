...
public class ExibirFuncionario extends javax.swing.JFrame {
    ...
    public  void pesquisar() {
        ...
        String[] colunas = new String[]{"ID", "NOME", "CPF", "SEXO", "FUNÇÃO", "ENDEREÇO", "DATANASC", "TELEFONE", "SALÁRIO"};
        DefaultTableModel listModel = new DefaultTableModel(colunas, 0);

        List<Funcionario> ls = new FuncionarioValid().listar();
        for (Funcionario c : ls) {
            
            listModel.addRow(new Object[]{c.getId(), c.getNome(), c.getCpf(), c.getSexo(), c.getFuncao(), c.getEndereco(), c.getDatanas(), c.getTelefone(), c.getSalario()});

        }

        jTabelafuncionario.setModel(listModel);
   ...
   int indice = jTabelafuncionario.getSelectedRow();
        if (indice == -1) {
            return;
...
//// /recupera o indice da linha selecionada
                ...
                //// apaga o registro
                new FuncionarioValid().excluir(idFuncionario);
                //ATUALIZA A LISTA
                pesquisar();
